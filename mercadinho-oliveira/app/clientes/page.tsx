...
export default function Clientes(){
  const[form,setForm]=useState<any>(vazio);const[busca,setBusca]=useState('');const[clientes,setClientes]=useState<any[]>([]);const[fiados,setFiados]=useState<any[]>([]);const[vendas,setVendas]=useState<any[]>([]);const[selecionado,setSelecionado]=useState<any>(null)
  async function load(){await requireUser();const[c,f,v]=await Promise.all([supabase.from('clientes').select('*').order('nome'),supabase.from('fiados').select('*').order('created_at',{ascending:false}),supabase.from('vendas').select('*').order('created_at',{ascending:false}).limit(250)]);setClientes(c.data||[]);setFiados(f.data||[]);setVendas(v.data||[])}
  useEffect(()=>{load()},[])
  async function salvar(e:any){e.preventDefault();if(!form.nome.trim())return alert('Informe o nome.');const payload={nome:form.nome.trim(),telefone:form.telefone.trim()||null,limite_fiado:onlyNumber(form.limite_fiado)};const resp=form.id?await supabase.from('clientes').update(payload).eq('id',form.id):await supabase.from('clientes').insert(payload);if(resp.error)return alert(resp.error.message);setForm(vazio);load()}
  ...
  async function quitar(f:any){const valor=Number(f.valor_aberto ?? f.valor ?? 0); if(!confirm(`Confirmar pagamento de ${brl(valor)}?`))return; await supabase.from('pagamentos_fiado').insert({fiado_id:f.id,cliente_id:f.cliente_id,valor,forma_pagamento:'dinheiro'}); await supabase.from('fiados').update({status:'quitado',valor_aberto:0}).eq('id',f.id); load()}
  ...
  return <main className="page"><div className="row"><div><h1 className="title">Clientes</h1><p className="subtitle">Cadastro, fiado e histórico de compras.</p></div><span className="pill"><Users size={15}/> {clientes.length}</span></div><div className="grid2 mt-4"><div className="card"><small>Fiado aberto</small><div className="money">{brl(totalAberto)}</div></div><div className="card"><small>Clientes devendo</small><div className="money">{new Set(fiados.filter(f=>f.status==='aberto').map(f=>f.cliente_id||f.cliente_nome)).size}</div></div></div>
