...
export default function Dashboard(){
  ...
  async function load(){
    setLoading(true)
    await requireUser()
    const[vh,vm,p,f,cp]=await Promise.all([
    ...
    setVendasHoje(vh.data||[]); setVendasMes(vm.data||[]); setProdutos(p.data||[]); setFiados(f.data||[]); setContasPagar(cp.data||[])
    setLoading(false)
  }
  useEffect(()=>{load()},[])
  const totalHoje=useMemo(()=>vendasHoje.reduce((a,b)=>a+Number(b.total||0),0),[vendasHoje])
  const totalMes=useMemo(()=>vendasMes.reduce((a,b)=>a+Number(b.total||0),0),[vendasMes])
  ...
  const chart=useMemo(()=>{
    ...
    for(let i=6;i>=0;i--){const d=new Date(); d.setDate(d.getDate()-i); d.setHours(0,0,0,0); const fim=new Date(d); fim.setDate(fim.getDate()+1); const value=vendasMes.filter(v=>{const dt=new Date(v.created_at); return dt>=d&&dt<fim}).reduce((a,v)=>a+Number(v.total||0),0); arr.push({label:labelDia(d),value})}
    return arr
  },[vendasMes])
